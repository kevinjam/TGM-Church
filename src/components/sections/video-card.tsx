...
import { Play, Calendar, User, Clock, ExternalLink } from "lucide-react"
...
export function VideoCard({ sermon, delay = 0 }: VideoCardProps) {
          ...
          {sermon.youtubeUrl ? (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="relative w-full h-full">
                <img
                  src={sermon.thumbnail || '/images/placeholder-sermon.jpg'}
                  alt={sermon.title}
                  className="w-full h-full object-cover opacity-80 group-hover:opacity-60 transition-opacity duration-300"
                />
                <div className="absolute inset-0 bg-black/20 group-hover:bg-black/10 transition-colors duration-300" />
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-16 h-16 bg-tgm-gold/90 rounded-full flex items-center justify-center group-hover:bg-tgm-gold group-hover:scale-110 transition-all duration-300 shadow-lg">
                    <Play className="h-6 w-6 text-tgm-blue ml-1" />
                  </div>
                </div>
              </div>
            </div>
              ...
              {sermon.youtubeUrl ? (
                <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                  <DialogTrigger asChild>
                    <Button className="flex-1 bg-tgm-gold text-tgm-blue hover:bg-tgm-lightgold transition-colors duration-300">
