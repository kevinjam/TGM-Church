...
import { SERMON_CATEGORIES } from "@/lib/db/constants"
import type { SermonView } from "@/lib/db/services/sermon"

const FILTER_CATEGORIES = ["All", ...SERMON_CATEGORIES]

export function SermonsView({ sermons }: { sermons: SermonView[] }) {
  const [selectedCategory, setSelectedCategory] = useState<string>("All")
  const [activeTab, setActiveTab] = useState<"all" | "video" | "audio">("all")
  const [currentlyPlaying, setCurrentlyPlaying] = useState<SermonView | null>(null)

  const matchesCategory = (sermon: SermonView) =>
    selectedCategory === "All" || sermon.category === selectedCategory

  const videoSermons = sermons.filter(
    (sermon) =>
      (sermon.type === "video" || sermon.type === "both") && matchesCategory(sermon)
  )
  const audioSermons = sermons.filter(
    (sermon) =>
      (sermon.type === "audio" || sermon.type === "both") && matchesCategory(sermon)
  )

  const handlePlayAudio = (sermon: SermonView) => {
